...
const userName = process.env.FCLI_USER || 'ts-example2'
...
const setupFindyAgency = async () => {
    ...
    console.log(grpcProps, authenticator);
    ...
    await agentClient.startListeningWithHandler(
        {
            ...
            DIDExchangeDone: async (info) => {
                ...
                // If connection was for issuing, continue by issuing the "wallet" credential
                // Wallet credential "walletAddress" attribute is the recipient walletAddress
                if (invitations.issue.includes(info.connectionId)) {
                    ...
                    console.log(`Creating a credential for wallet 0x6Dadd8140b081558537E3a519a1AfABebb3de697`);
                    attr.setName("walletAddress")
                    attr.setValue("0x6Dadd8140b081558537E3a519a1AfABebb3de697")
            ...
            PresentProofDone: (info) => {
                ...
                fetch(`https://asdf-ashy.vercel.app/api/claim-after-verification`, {
                    method: "POST",
                    headers: {
                      "Content-Type": "application/json",
                    },
                    body: JSON.stringify({
                      recipient: "0x6Dadd8140b081558537E3a519a1AfABebb3de697",
                    }),
                })
